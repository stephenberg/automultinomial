...
This vignette explains the installation and use of the R package automultinomial. The automultinomial package is designed to be used for regressions similar to logistic or multinomial logit regression. However, unlike ordinary logistic and multinomial logit models, the auto-logistic (multinomial) model includes an autocorrelation parameter to account for spatial dependence between observations.
...
4. Technical appendix-a comparison of different parameterizations of the 2-category autologistic model

#1 The problem automultinomial solves

Consider a data problem where covariates (the independent variables) and categorical responses (the dependent variables) are observed on a spatial grid or lattice. To describe the setup in automultinomial, we will frequently use the notations
...
We will also define an *energy* function $H(\mathbf{z}|\mathbf{\beta},\gamma)$. When $K=2$ and the two categories are $k=1$ and $k=2$, then
$\mathbf{\beta}$ is just a vector with length $p$, and \begin{eqnarray}
H(\mathbf{z}|\beta,\gamma)=\sum_{i=1}^{n}x_i^T\beta I(z_i=2)+\gamma \sum_{i=1}^{n}\sum_{\substack{i'\sim i\\i'>i}}\sum_{k=1}^{2}I(z_i=z_{i'}=k)
\end{eqnarray}

In general, for $K\geq 2$ we define $H(\mathbf{z}|\mathbf{\beta},\gamma)$ by
...
\begin{eqnarray}
p(\mathbf{z}|\mathbf{\beta},\gamma)=\frac{\exp\{H(\mathbf{z}|\mathbf{\beta},\gamma)\}}{\sum_{\mathbf{z'}}\exp\{H(\mathbf{z'}|\mathbf{\beta},\gamma)\}}\label{density}
\end{eqnarray}
...
###Estimating $\mathbf{\beta}$ and $\gamma$
...
\begin{eqnarray}
\ell_{PL}=\sum_{i=1}^{n}\log\{p(z_i|z_{-i},\mathbf{\beta},\gamma)\}\label{pseudolikelihood}
\end{eqnarray}

over $\beta$ and $\gamma$. The expression $p(z_i|z_{-i},\mathbf{\beta},\gamma)$ refers to the conditional density of $z_i$ given the sites at all of the other grid locations. A short calculation based on \eqref{density} shows that for $K=2$

\begin{eqnarray}
p(z_i=1|z_{-i},\mathbf{\beta},\gamma)&=&\frac{\exp\{\gamma\sum_{i'\sim i}I(z_{i'}=1)\}}{\exp\{\gamma\sum_{i'\sim i}I(z_{i'}=1)\}+\exp\{x_i^T\beta I(z_i=2)+\gamma\sum_{i'\sim i}I(z_{i'}=2)\}}\label{p1}\\
p(z_i=2|z_{-i},\mathbf{\beta},\gamma)&=&\frac{\exp\{x_i^T\beta I(z_i=2)+\gamma\sum_{i'\sim i}I(z_{i'}=2)\}}{\exp\{\gamma\sum_{i'\sim i}I(z_{i'}=1)\}+\sum_{k=1}^{K-1}\exp\{x_k^T\beta I(z_i=2)+\gamma\sum_{i'\sim i}I(z_{i'}=2)\}}\label{p2}
\end{eqnarray}
...
\begin{eqnarray}
p(z_i=1|z_{-i},\mathbf{\beta},\gamma)&=&\frac{\exp\{\gamma\sum_{i'\sim i}I(z_{i'}=1)\}}{\exp\{\gamma\sum_{i'\sim i}I(z_{i'}=1)\}+\sum_{k=1}^{K-1}\exp\{x_k^T\beta_kI(z_i=k+1)+\gamma\sum_{i'\sim i}I(z_{i'}=k+1)\}}\label{p3}
\end{eqnarray}

and for $k>1$,
\begin{eqnarray}
p(z_i=k|z_{-i},\mathbf{\beta},\gamma)&=&\frac{\exp\{x_i^T\beta_kI(z_i=k+1)+\gamma\sum_{i'\sim i}I(z_{i'}=k+1)\}\}}{\exp\{\gamma\sum_{i'\sim i}I(z_{i'}=1)\}+\sum_{k=1}^{K-1}\exp\{x_k^T\beta_kI(z_i=k+1)+\gamma\sum_{i'\sim i}I(z_{i'}=k+1)\}}\label{p4}
\end{eqnarray}
...
Below is a plot of the responses on the grid. On the left plot, we can see "clumping" of the responses due to the positive autocorrelation parameter $\gamma=0.7$. The right hand plot is from the distribution with the same $\beta$, and $\gamma=0$.
...
Below is a plot of the responses on the grid. On the left plot, we again see clumping of the responses due to the positive autocorrelation parameter $\gamma=0.7$. The right hand plot is from the distribution with the same $\beta$, and $\gamma=0$.
...
From the figure below, we see that our pseudolikelihood parameter estimates are close to the true coefficient values. Additionally, the bootstrap and asymptotic confidence intervals are extremely similar, so it would be reasonable, at least  for this example, to save computational time and use the asymptotic intervals.
...
df=as.data.frame(df)
df=cbind(df,factor(c(as.character(1:10),"gamma")))
colnames(df)[5:7]=c("PMLE","Truth","coefficient")
cbPalette <- c("Asymptotic"="#009E73" ,"Bootstrap"="#CC79A7")
...
p1=p1+geom_point(data=df,aes(x=coefficient,y=PMLE),size=2,shape=21,fill="black")
p1=p1+geom_point(data=df,aes(x=coefficient,y=Truth),size=2, shape=21, fill="white")
p1=p1+scale_y_continuous(limits=c(-0.4,0.8),name = "Coefficient Estimates")
