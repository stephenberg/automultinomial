...
* $\bm{\beta}\in\mathbb{R}^{p\times (K-1)}$ to denote the coefficient matrix (when $K=2$, $\beta$ is a vector of length $p$, as in logistic regression)
...
We will also define an *energy* function $H(\mathbf{z}|\bm{\beta},\gamma)$. When $K=2$ and the two categories are $k=1$ and $k=2$, then
$\bm{\beta}$ is just a vector with length $p$, and \begin{eqnarray}
H(\mathbf{z}|\beta,\gamma)=\sum_{i=1}^{n}x_i^T\beta I(z_i=2)+\gamma \sum_{i=1}^{n}\sum_{\substack{i'\sim i\\i'>i}}\sum_{k=1}^{2}I(z_i=z_{i'}=k)
\end{eqnarray}

In general, for $K\geq 2$ we define $H(\mathbf{z}|\bm{\beta},\gamma)$ by
...
\begin{eqnarray}
p(\mathbf{z}|\bm{\beta},\gamma)=\frac{\exp\{H(\mathbf{z}|\bm{\beta},\gamma)\}}{\sum_{\mathbf{z'}}\exp\{H(\mathbf{z'}|\bm{\beta},\gamma)\}}\label{density}
\end{eqnarray}
...
\begin{eqnarray}
\ell_{PL}=\sum_{i=1}^{n}\log\{p(z_i|z_{-i},\bm{\beta},\gamma)\}\label{pseudolikelihood}
\end{eqnarray}

over $\beta$ and $\gamma$. The expression $p(z_i|z_{-i},\bm{\beta},\gamma)$ refers to the conditional density of $z_i$ given the sites at all of the other grid locations. A short calculation based on \eqref{density} shows that for $K=2$

\begin{eqnarray}
p(z_i=1|z_{-i},\bm{\beta},\gamma)&=&\frac{\exp\{\gamma\sum_{i'\sim i}I(z_{i'}=1)\}}{\exp\{\gamma\sum_{i'\sim i}I(z_{i'}=1)\}+\exp\{x_i^T\beta I(z_i=2)+\gamma\sum_{i'\sim i}I(z_{i'}=2)\}}\label{p1}\\
p(z_i=2|z_{-i},\bm{\beta},\gamma)&=&\frac{\exp\{x_i^T\beta I(z_i=2)+\gamma\sum_{i'\sim i}I(z_{i'}=2)\}}{\exp\{\gamma\sum_{i'\sim i}I(z_{i'}=1)\}+\sum_{k=1}^{K-1}\exp\{x_k^T\beta I(z_i=2)+\gamma\sum_{i'\sim i}I(z_{i'}=2)\}}\label{p2}
\end{eqnarray}
...
\begin{eqnarray}
p(z_i=1|z_{-i},\bm{\beta},\gamma)&=&\frac{\exp\{\gamma\sum_{i'\sim i}I(z_{i'}=1)\}}{\exp\{\gamma\sum_{i'\sim i}I(z_{i'}=1)\}+\sum_{k=1}^{K-1}\exp\{x_k^T\beta_kI(z_i=k+1)+\gamma\sum_{i'\sim i}I(z_{i'}=k+1)\}}\label{p3}
\end{eqnarray}

and for $k>1$,
\begin{eqnarray}
p(z_i=k|z_{-i},\bm{\beta},\gamma)&=&\frac{\exp\{x_i^T\beta_kI(z_i=k+1)+\gamma\sum_{i'\sim i}I(z_{i'}=k+1)\}\}}{\exp\{\gamma\sum_{i'\sim i}I(z_{i'}=1)\}+\sum_{k=1}^{K-1}\exp\{x_k^T\beta_kI(z_i=k+1)+\gamma\sum_{i'\sim i}I(z_{i'}=k+1)\}}\label{p4}
\end{eqnarray}
